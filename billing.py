...
class BillClass:
    def __init__(self,root):
        ...
        btn_search=Button(ProductFrame2,text="Search",command=self.search,font=("goudy old style",15,"bold"),bg="#2196f3",fg="white",cursor="hand2").place(x=285,y=45,width=100,height=25)
        btn_show_all=Button(ProductFrame2,text="Show All",command=self.show,font=("goudy old style",15,"bold"),bg="#083531",fg="white",cursor="hand2").place(x=285,y=10,width=100,height=25)

        #===Product Detail Frame===
        ProductFrame3=Frame(ProductFrame1,bd=3,relief=RIDGE)
        ProductFrame3.place(x=2,y=140,width=390,height=375)
        ...
        txt_p_price=Entry(Add_CartWidgetsFrame,textvariable=self.var_price,font=("times new roman",13),bg="lightyellow",state='readonly').place(x=230,y=35,width=150,height=22)

        lbl_p_qty=Label(Add_CartWidgetsFrame,text="Quantity",font=("times new roman",15),bg="white").place(x=390,y=5)
        txt_p_qty=Entry(Add_CartWidgetsFrame,textvariable=self.var_qty,font=("times new roman",13),bg="lightyellow").place(x=390,y=35,width=120,height=22)
        ...
        btn_print=Button(billMenuFrame,text='Print Bill',command=self.print_bill,cursor="hand2",font=("goudy old style",15,"bold"),bg="lightgreen",fg="white")
        ...
        btn_genrate=Button(billMenuFrame,text='Generate/Save Bill',command=self.generate_bill,cursor="hand2",font=("goudy old style",15,"bold"),bg="#009688",fg="white")
        btn_genrate.place(x=246,y=80,width=160,height=50)

        #====Footer=====
        footer=Label(self.root,text="FSC-Factory Stock Checking & Billing System\nDeveloped By HVC",font=("times new roman",11),bg="#4d636d",fg="white").pack(side=BOTTOM,fill=X)
    ...
    def generate_bill(self):
        if self.var_cname.get()=='' or self.var_contact.get()=='':
            ...
            fp.close()
            messagebox.showinfo('Saved',"Bill has been saved successfully",parent=self.root)
            self.chk_print=1
    ...
    def bill_top(self):
        ...
        bill_top_temp=f'''
\t\tXYZ-Inventory
\t Phone No. 88884***** , Jalgaon-425001 
{str("="*47)}
 Customer Name :  {self.var_cname.get()}
 Contact Number :   {self.var_contact.get()}
 Bill No. : {str(self.invoice)}\t\t\tDate : {str(time.strftime("%d/ %B / %Y"))}
{str("="*47)}
